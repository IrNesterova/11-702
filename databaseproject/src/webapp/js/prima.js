...
var InitialView = Backbone.View.extend({
	...
	submitForm : function(){
		...
		} else if (!$('#employeereserved').val()){
            $(".error").html("Employee is required");
		}
		else
		{
			$(".error").html("");
...
var ScreenUI=Backbone.View.extend({
	...
	updateTicketInfo:function(){
		...
		$("#ticket-sold-info").append("<tr><td>"+$('#name').val()+"</td><td>"+$('#seats').val()
			+"</td><td>"+bookedSeats+"</td><td>"+$('#employeereserved').val()+"</td><td>"+$('#ReservationContact').val()+"</td></tr>");
	},
...
var TicketInfo=Backbone.View.extend({
	initialize:function(){
		var items=[];
		var items1 = {key1:["val1"], key2:["val2", "val3"]};
